...
class SplitGD():
    ...
    def __init__(self, keras_model):
        if keras_model: self.model = keras_model
        else: self.model = tf.keras.Sequential()
    ...
    def modify_gradients(self,gradients, lrate):   
        
        # Loop through layers where the dL/dw is, and update it to the dV/dw
        for layer in range( 0, len(gradients), 2):
            for node in gradients[layer].numpy():
                node[:] = [ lw * -2*self.get_td(self.loss) for lw in node ]
    ...
    # value of a tensor.
    def gen_loss(self,features,targets):
        predictions = self.model(features)  # Feed-forward pass to produce outputs/predictions
        loss = tf.keras.losses.MeanSquaredError() #self.model.loss(targets, predictions) # model.loss = the loss function
        return loss(targets, predictions)

    def fit(self, features, targets, lrate, epochs=1, mbs=1,vfrac=0.1,verbosity=1,callbacks=[]):
        ...
        for epoch in range(epochs):
            ...
            for _ in range(math.floor(len(train_ins) / mbs)):
                with tf.GradientTape() as tape:  # Read up on tf.GradientTape !!
                    ...
                    self.loss = self.gen_loss(feaset,tarset)
                    
                    # Get the derivative of loss with respect to weights
                    ...
                    gradients = self.modify_gradients(gradients, lrate)  
    ...
    def get_weights(self, weights):
        temp = []
        # Grabbing the values that are stored inside the numpy array
        for a_w in range(0, len(weights),2):
            temp.append( weights[a_w].numpy() )
        
        return temp

    # returns the trainign error aka loss
    def get_td(self, loss):
        return loss.numpy()

    
    # Initializes elig rates with the same shape as number of nodes
    def init_elig(self, layers):
        for l in layers[:len(layers) - 1]:
            self.eligibilities.append([1])
            for u in range(len(l)):
                self.eligibilities[u].append([1])
